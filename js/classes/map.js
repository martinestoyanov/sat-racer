class Map {
  constructor(map, mapX, mapY) {
    ...
    this.checkpointAnnounce = document.getElementById("checkpoint");
    this.mapX = 2295;
    this.mapY = 4030;
    this.checkpoints = [
      {
  ...
  checkpointTracker() {
    ...
    map.checkpoints.forEach((element, i, arr) => {
      if (
        ...
        this.checkpointAnnounce.innerHTML = `Checkpoint ${i} passed!`;
        console.log(`Checkpoint ${i} passed!`);
        setTimeout(() => {
          this.checkpointAnnounce.innerHTML = "";
        }, 2000);
      }
    });
  }
  ...
  findCheckpointCenter(checkpoint) {
    map.resetCheckpoint.x =
      (checkpoint.x1 + checkpoint.x2) / 2 - canvas.width / 4;
    map.resetCheckpoint.y =
      (checkpoint.y1 + checkpoint.y2) / 2 - canvas.height / 4;
